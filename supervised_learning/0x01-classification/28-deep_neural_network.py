...
class DeepNeuralNetwork:
    def __init__(self, nx, layers, activation = "sig"):
        if type(nx) != int:
            ...
            raise ValueError("activation must be a sig or tanh")
    ...
    def forward_prop(self, X):
        ...
        input_of_layer = X
        while i <= self.__L:
            Z = numpy.matmul(self.__weights["W{0}".format(i)], input_of_layer) + self.__weights["b{0}".format(i)]
            if i == self.__L:
                 self.__cache["A{0}".format(i)] = numpy.exp(Z) / numpy.sum(numpy.exp(Z), axis = 0, keepdims = True)
            else:
                if self.__activation == "sig":
                    self.__cache["A{0}".format(i)] = 1 / (1 + numpy.exp(-1 * Z))
                else:
                    self.__cache["A{0}".format(i)] = numpy.tanh(Z)
            input_of_layer = self.__cache["A{0}".format(i)]
            i += 1
        return self.__cache["A{0}".format(i - 1)], self.__cache

    def cost(self, Y, A):
        return -1 * numpy.sum(Y * numpy.log(A)) / Y.shape[1]
        ...
        while i >= 1:
            if i == self.__L:
                ...
                if self.__activation == "sig":
                    ...
                    Dcost_by_Dz = numpy.matmul(W_prev.T, Dcost_by_Dz) * (1 - (cache["A{0}".format(i)] ** 2))
                W_prev = numpy.copy(self.__weights["W{0}".format(i)])
    ...
    def train(self, X, Y, iterations = 5000, alpha = 0.05, verbose = True, graph = True, step = 100):
        ...
        while i <= iterations:
            ...
            if verbose and i % step == 0:
                x.append(i)
                cost.append(self.cost(Y, self.__cache["A{0}".format(self.__L)]))
                print("Cost after {0} iterations: {1}".format(i, cost[-1]))
