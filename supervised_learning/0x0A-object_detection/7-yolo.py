...
class Yolo:
    ...
    def predict(self, folder_path):
        ...
        for i in range(pimages.shape[0]):
            ## Get a single image with different grid sizes
            outp = [output[i] for output in outputs]
            ## Get the processed output of a single image
            boxes, box_confidences, box_class_probs = (
                                    self.process_outputs(outp, image_shapes[i]))
            ## Get boxes with relativily higher scores
            boxes, box_classes, box_scores = (
                                     self.filter_boxes(boxes, box_confidences, box_class_probs))
            ## Get boxes with absolutely higher scores from
            ...
            ## Append the predictions of a single image in an array
            predictions.append((boxes, box_classes, box_scores))
            ## Display the image with it's corrosponding predictions
